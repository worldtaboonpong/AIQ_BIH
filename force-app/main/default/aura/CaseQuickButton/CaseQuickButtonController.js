({
    doInit : function(component, event, helper) {
        ...
        action.setCallback(this, function(data) {
            ...
            if (CaseInfo[0].Age_Formula__c < 16){
            ...
            helper.getisPC(component, event, helper);
    ...
    doMinorCheck : function(component, event, helper) {
        var recordId = component.get("v.recordId");
        var action = component.get("c.getCaseInfo");
        action.setParams({ "caseId": recordId});
        action.setCallback(this, function(data) {
	...
	navigateToGeneratePDF: function (component, event, helper) {
		...
		var pageReference = {
			...
			attributes: {
				componentName: "c:BIH_GenerateInvitionLetter",
			},
			state: {
...
onCheck2: function(component, event, helper) {
 ...
 closeCase: function(component, event, helper) {
        ...
        // Call Apex method to close the case
        var action = component.get("c.closeCaseByMyselfAction");
        action.setParams({
            "caseId": caseId
        });
        ...
        action.setCallback(this, function(response) {
            ...
            if (state === "SUCCESS") {
                ...
                toastEvent.setParams({
                    "title": "Success!",
                    "message": "Case closed by myself successfully.",
                    "type": "success"
                });
                toastEvent.fire();
                ...
                // Handle errors
                console.log('Error closing case: ' + response.getError());
            }
        });

        $A.enqueueAction(action);
    },
 closeCaseSupport: function(component, event, helper) {
        var caseId = component.get("v.recordId");

        // Call Apex method to close the case
        var action = component.get("c.closeCaseSuppotAction");
        action.setParams({
            "caseId": caseId
        });
        ...
        action.setCallback(this, function(response) {
            ...
            if (state === "SUCCESS") {
                ...
                toastEvent.setParams({
                    "title": "Success!",
                    "message": "Case closed with support successfully.",
                    "type": "success"
                });
                toastEvent.fire();
